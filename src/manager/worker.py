...
class WorkerEngine:
  ...
  async def push(self, msg: Message):
    ...
    if msg.type == "QUERY":
      ...
      hello_msg = Message (0, "HELLO", {"worker_id": self.id, "hardware_platform": self.hardware_platform, "gpu_name": self.gpu_name, "total_mem": self.total_mem})
      await self.outgoing[0].send(hello_msg)
      self.logger = Logger(
        header=["timestamp", "in_sys_timestamp", "worker_id", "variant_id", "name", "throughput", "batch_size", "qsize" ],
        directory=self.config["parameters"]["log_dir"],
        filename="worker-{}.csv".format(self.id),
        )
      await self.logger.write(write_header=True)
    else:
  ...
  async def deployment_daemon(self):
    try:
      while True:
        ...
        self.tasks[model_.id] = asyncio.get_running_loop().run_in_executor(None, self.run_inference, model_.id)
  ...
  async def start(self):
    tasks = [ self.monitor_incoming_data(), self.monitor_daemon(), self.logger_daemon(), self.deployment_daemon(), self.stop_daemon() ]
