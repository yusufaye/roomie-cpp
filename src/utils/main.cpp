...
int main(int argc, char const *argv[])
{
  ...
  if (argc > 1)
  {
    ...
    {
      path_arg = "src/config/infaas/query_generator-howsrv-3.json";
      engine = new PoissonZipfQueryGenerator();
    }
    else if (engine_arg == "C" || engine_arg == "c")
    {
      path_arg = "src/config/infaas/controller-howsrv-3.json";
      engine = new Controller(Approach::INFAAS);
    }
    else if (engine_arg == "W" || engine_arg == "w")
    {
      path_arg = "src/config/infaas/worker_howsrv-3.json";
